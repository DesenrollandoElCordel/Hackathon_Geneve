...
setwd("/Users/pauline/Documents/Hackathon_Cordel")
```
...
```{r}
list_of_files <- list.files(path = "./topics", recursive = TRUE,
                            pattern = "\\.txt$", 
                            full.names = TRUE)
...
```{r}
udmodel <- udpipe_load_model(file = 'models/spanish-ancora-ud-2.1-20180111.udpipe')
textLemmat <- ""
increment<-0
mybiglist <- list()
...
best <- TRUE
#7 topics
lda_gibbs_29 <- LDA(dtm_clean, 29, method="Gibbs", control=list(nstart=nstart, seed=seed, best=best, burnin=burnin, iter=iter, thin=thin))
```
...
```{r}
"LDA 2"
termsTopic <- as.data.frame(terms(lda_gibbs_29,10))
head(termsTopic,11)
```
...
```{r}
# load raw data
full_data <- paste(scan("/Users/pauline/Documents/Hackathon_Cordel/TXT_moreno/Full_moreno_1.txt", what="character", sep="", fileEncoding="UTF-8"),collapse=" ")
...
```{r}
udmodel <- udpipe_load_model(file = '/Users/pauline/Documents/Hackathon_Cordel/models/spanish-ancora-ud-2.1-20180111.udpipe')
```
...
textLemmat <- ""
#apply the model
x <- udpipe_annotate(udmodel, x = full_data)
#return the result as dataframe
x <- as.data.frame(x)
#Now I extract the column of the lemma
 textLemmat <-x$lemma[1]
...
mesStops="stopwords-es.csv"
#indiquer l'URL où se trouve le document à télécharger
stopword_enLigne = "https://raw.githubusercontent.com/stopwords-iso/stopwords-es/master/stopwords-es.txt"
...
stopword_enLigne = read.csv("stopwords-es.csv", header=FALSE, stringsAsFactors=FALSE)[,]
full_data_lemmat<-removeWords(full_data_lemmat, stopword_enLigne)
```
...
```{r}
#clean corpus
full_data_clean <- full_data_lemmat %>%
  stringr::str_to_title()
# tokenize corpus
full_data_tokzd <- quanteda::tokens(full_data_clean)
# extract bigrams
BiGrams <- full_data_tokzd %>% 
...
```{r}
ngram_extract <- quanteda::tokens_compound(full_data_tokzd, pattern = BiGrams)
...
```{r}
full_data_sentences_lemmatised<-gsub("TRUC_A_MODIFIER", "TRUC_MODIFIE", full_data_sentences_lemmatised)
full_data_sentences_lemmatised <- full_data_sentences_lemmatised %>%
  #remove useless whitespaces
  stringr::str_squish()
```
...
```{r}
full_data_sentences_lemmatised.txt <- readLines("full_data_sentences_lemmatised.txt", encoding="UTF-8")
```
